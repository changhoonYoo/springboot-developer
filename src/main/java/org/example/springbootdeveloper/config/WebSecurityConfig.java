...
public class WebSecurityConfig {
    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        return http
                ...
                .requestMatchers("/login", "/signup", "/user",
                        ...
                        "/v3/api-docs/**"
                ).permitAll() // 특정 요청과 일치하는 url에 대한 엑세스 설정 (누구나 접근 가능)
    ...
    // 인증 관리자 관련 설정 (사용자 정보를 가져올 서비스 재정의 or 인증 방법(LDAP, JDBC) 기반 인증 등을 설정
    @Bean
    public AuthenticationManager authenticationManager(HttpSecurity http,
                                                       BCryptPasswordEncoder bCryptPasswordEncoder,
                                                       UserDetailService userDetailService) throws Exception {
        return http.getSharedObject(AuthenticationManagerBuilder.class)
                .userDetailsService(userDetailService) // 사용자 정보를 가져올 서비스 설정(반드시 UserDetailsService 상속받은 클래스)
